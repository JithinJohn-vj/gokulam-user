...
import { checkoutData, dineInData } from './Services';
...
export function useDineInData() {
  const router = useRouter();
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => dineInData(data),
    onMutate: () => {
      console.log('mutate');
    },
...
export function useCheckoutData() {
  ...
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => checkoutData(data),
    onMutate: () => {
      console.log('mutate');
    },
    onSettled: async (_, error) => {
      if (error) {
        ...
        toast.success('order complete ,redirecting to payment ....');
        // const expiryTime = Date.now() + 2 * 60 * 1000; // 2 minutes from now
        // router.push(`/otp-verify?expiry=${expiryTime}`)
        await queryClient.invalidateQueries({ queryKey: ['getAgent'] });
      }
